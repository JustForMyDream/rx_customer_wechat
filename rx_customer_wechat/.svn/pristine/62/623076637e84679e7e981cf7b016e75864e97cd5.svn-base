...
<script>
    ...
    window.onload=function () {
        var h=screen.height;
        var w=screen.width;
        alert(Math.sqrt(h*h+w*w));
        var size=Math.sqrt(h*h+w*w)/765*16;
        document.getElementsByTagName('html')[0].style.fontSize=size+'px';
    }
    
    var rxPhone="<%=res.getString("rxPhone")%>";
